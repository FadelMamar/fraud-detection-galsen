...
def threshold_based_metrics(fraud_probabilities, true_label, thresholds_list):
    ...
    for threshold in thresholds_list:
        predicted_classes = get_class_from_fraud_probability(
            fraud_probabilities, threshold=threshold
        )

        (TN, FP, FN, TP) = metrics.confusion_matrix(
            true_label, predicted_classes
        ).ravel()
...
def model_selection_performances(
    performances_df_dictionary, performance_metric="AUC ROC"
):
    ...
    for model_class, performances_df in performances_df_dictionary.items():
        ...
        default_performances = performances_df[
            ...
            == default_parameters_dictionary[model_class]
        ]
        ...
        performances_summary = get_summary_performances(
            performances_df, parameter_column_name="Parameters summary"
        )
        mean_std_performances = performances_summary.loc[["Test performance"]][
            performance_metric
        ].values[0]
        mean_std_performances = mean_std_performances.split("+/-")
        mean_performances_dictionary["Best validation parameters"].append(
            float(mean_std_performances[0])
        )
        std_performances_dictionary["Best validation parameters"].append(
            float(mean_std_performances[1])
        )

        # Get the performances for the boptimal parameters
        mean_std_performances = performances_summary.loc[["Optimal test performance"]][
            performance_metric
        ].values[0]
        mean_std_performances = mean_std_performances.split("+/-")
        mean_performances_dictionary["Optimal parameters"].append(
            float(mean_std_performances[0])
        )
        std_performances_dictionary["Optimal parameters"].append(
            float(mean_std_performances[1])
        )
...
def kfold_cv_with_classifier(
    classifier, X, y, n_splits=5, strategy_name="Basline classifier"
):
    ...
    results_df = pd.DataFrame(
        [
            [
                str(round(results_mean[i], 3)) + "+/-" + str(round(results_std[i], 3))
    ...
    train_df = pd.DataFrame(
        {"X1": X[train_index, 0], "X2": X[train_index, 1], "Y": y[train_index]}
    )
    test_df = pd.DataFrame(
        {"X1": X[test_index, 0], "X2": X[test_index, 1], "Y": y[test_index]}
    )
...
def get_template_tx_stats(
    ...
    ax.axvline(
        start_date_training + datetime.timedelta(days=delta_train + delta_delay),
        0,
        ylim,
        color="black",
    )
    ...
    ax.text(
        start_date_training + datetime.timedelta(days=2),
        ylim - 20,
        "Training period",
        fontsize=fs,
    )
    ax.text(
        start_date_training + datetime.timedelta(days=delta_train + 2),
        ylim - 20,
        "Delay period",
        fontsize=fs,
    )
    ax.text(
        start_date_training + datetime.timedelta(days=delta_train + delta_delay + 2),
        ylim - 20,
        "Test period",
        fontsize=fs,
    )
...
def get_template_pr_curve(ax, title, fs, baseline=0.5):
    ...
    ax.plot(
        ...
        [baseline, baseline],
        "r--",
        label="AP Random = {0:0.3f}".format(baseline),
    )
...
# Get the performance plot for a single performance metric
def get_execution_times_plot(
    performances_df, title="", parameter_name="Tree maximum depth"
):
    ...
    ax.plot(
        performances_df["Parameters summary"],
        performances_df["Execution time"],
        color="black",
    )
...
def get_model_selection_performance_plot(
    ...
    ax.bar(
        r1,
        mean_performances_dictionary["Default parameters"],
        width=barWidth,
        color="#CA8035",
        edgecolor="black",
        yerr=std_performances_dictionary["Default parameters"],
        capsize=7,
    ...
    ax.bar(
        r2,
        mean_performances_dictionary["Best validation parameters"],
        width=barWidth,
        color="#008000",
        edgecolor="black",
        yerr=std_performances_dictionary["Best validation parameters"],
        capsize=7,
    ...
    ax.bar(
        r3,
        mean_performances_dictionary["Optimal parameters"],
        width=barWidth,
        color="#2F4D7E",
        edgecolor="black",
        yerr=std_performances_dictionary["Optimal parameters"],
        capsize=7,
...
def get_model_selection_performances_plots(
    performances_df_dictionary,
    performance_metrics_list=["AUC ROC", "Average precision", "Card Precision@100"],
    ...
    # Plot performance metric for each metric in performance_metrics_list
    for i in range(n_performance_metrics):
        get_model_selection_performance_plot(
            performances_df_dictionary,
            ax[i],
...
def plot_decision_boundary_classifier(
    ...
    x1_min, x1_max = (
        train_df[input_features[0]].min() - 1,
        train_df[input_features[0]].max() + 1,
    )
    x2_min, x2_max = (
        train_df[input_features[1]].min() - 1,
        train_df[input_features[1]].max() + 1,
    )

    plot_step = 0.1
    xx, yy = np.meshgrid(
        np.arange(x1_min, x1_max, plot_step), np.arange(x2_min, x2_max, plot_step)
    )
    ...
    if plot_training_data:
        ...
        for name, group in groups:
            ax.scatter(
                group[input_features[0]],
                group[input_features[1]],
                edgecolors="black",
                label=name,
            )
